...
function storeCarInfo(manufacturer, modelName) {
    ...
    var carInfo = "Manufacturer: '".concat(manufacturer, "',\n - ModelName: '").concat(modelName, "',\n");
    // Loop through the options and add them to the carInfo string
    options.forEach(function (option) {
        var key = option[0], value = option[1];
        carInfo += "- ".concat(key.charAt(0).toUpperCase() + key.slice(1), ": ").concat(value, ",\n");
    });
    return carInfo;
}
// Call the function for different car models and store their information
var car1 = storeCarInfo("Toyota", "Camry", ["color", "red"], ["year", 2022]);
var car2 = storeCarInfo("Honda", "Civic", ["color", "blue"], ["year", 2023], ["trim", "EX"]);
var car3 = storeCarInfo("Ford", "Mustang", ["color", "yellow"], ["year", 2022], ["engine", "V8"]);
var car4 = storeCarInfo("Tesla", "Model 3", ["color", "black"], ["year", 2023]);
var car5 = storeCarInfo("Chevrolet", "Silverado", ["color", "silver"], ["year", 2021], ["engine", "V6"]);
